...
- The presentation layer should always consist of the Presenter, UiState, EventHandler interface (which is implemented by the ViewModel/Presenter) and UiAction for side effects triggered by Presenter. This has one exception, and that is for cases where the logic does not have enough complexity to warrant this. Eg. just one side effect is not enough to create a UiAction or just one state is not enough to create a UiState
