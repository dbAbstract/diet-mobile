...
@Composable
internal fun SignInSheetScreen(viewModel: AuthViewModel = koinViewModel()) {
    val state by viewModel.uiState.collectAsStateWithLifecycle()
    val snackBarHostState = LocalSnackBarHostState.current

    SignInSheetContent(state = state, eventHandler = viewModel)
...
@Composable
internal fun SignInSheetContent(
    state: AuthUiState,
    ...
    Column(
        ...
        Box(
            ...
            TextButton(onClick = eventHandler::onForgotPasswordClicked) {
                Text(
        ...
        Button(
            onClick = {
                eventHandler.signInWithEmailAndPassword(
...
// isPasswordVisible is UI-local — kept inside this composable since nothing outside it needs to know.
@Composable
private fun SheetPasswordField(
    ...
    Column(
        ...
        TextField(
            ...
            // Masks characters when hidden; revealed on toggle.
            // Requires material-icons-extended for the toggle icon — add dependency in plumbing step.
...
@Composable
private fun SignInSheetLightPreview() {
    YaseyoPreview(darkTheme = false) {
        SignInSheetContent(state = previewAuthUiState, eventHandler = previewAuthEventHandler)
    }
}

@Preview(showBackground = true)
@Composable
private fun SignInSheetDarkPreview() {
...
@Preview(showBackground = true)
@Composable
private fun SignInSheetLoadingPreview() {
    YaseyoPreview(darkTheme = false) {
        SignInSheetContent(state = previewAuthUiState, eventHandler = previewAuthEventHandler)
    }
}
